...
FUNDAMENTAL_PERCENTAGE_INDICATORS = {
    "Div. Yield", "ROE", "Marg. Bruta", "Marg. EBIT", "Cres. Rec (5a)"
}

FUNDAMENTAL_INDICATOR_COLUMNS = (
    "pl", "p_vp", "dividend_yield", "roe", "divida_liquida_patrimonio",
    "margem_bruta", "margem_ebit", "crescimento_receita_5a"
)

def ensure_database_schema(cursor):
    """Cria as tabelas auxiliares usadas pela aplicação."""
    cursor.execute('''
    ...
    ''')
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS app_fundamentals (
            ...
            pl NUMERIC(14, 1),
            p_vp NUMERIC(14, 1),
            dividend_yield NUMERIC(14, 1),
            roe NUMERIC(14, 1),
            divida_liquida_patrimonio NUMERIC(14, 1),
            margem_bruta NUMERIC(14, 1),
            margem_ebit NUMERIC(14, 1),
            crescimento_receita_5a NUMERIC(14, 1)
        )
    ''')
    cursor.execute('ALTER TABLE app_fundamentals ADD COLUMN IF NOT EXISTS id BIGSERIAL')
    cursor.execute('ALTER TABLE app_fundamentals ADD COLUMN IF NOT EXISTS collected_date DATE')
    cursor.execute('ALTER TABLE app_fundamentals ADD COLUMN IF NOT EXISTS margem_bruta TEXT')
    cursor.execute('ALTER TABLE app_fundamentals ADD COLUMN IF NOT EXISTS margem_ebit TEXT')
    cursor.execute('ALTER TABLE app_fundamentals ADD COLUMN IF NOT EXISTS crescimento_receita_5a TEXT')
    for column in FUNDAMENTAL_INDICATOR_COLUMNS:
        cursor.execute(
            """
            SELECT data_type
            FROM information_schema.columns
            WHERE table_name = 'app_fundamentals' AND column_name = %s
            """,
            (column,)
        )
        column_type = cursor.fetchone()
        if column_type and column_type[0] in ("text", "character varying"):
            cursor.execute(
                f"""
                ALTER TABLE app_fundamentals
                ALTER COLUMN {column} TYPE NUMERIC(14, 1)
                USING CASE
                    WHEN NULLIF(TRIM({column}), '') IS NULL
                         OR TRIM({column}) IN ('-', 'N/A') THEN NULL
                    ELSE REPLACE(REPLACE(REPLACE(TRIM({column}), '%', ''), '.', ''), ',', '.')
                         ::NUMERIC(14, 1)
                END
                """
            )
    cursor.execute('UPDATE app_fundamentals SET collected_date = collected_at::date WHERE collected_date IS NULL')
    cursor.execute('ALTER TABLE app_fundamentals ALTER COLUMN collected_date SET DEFAULT CURRENT_DATE')
    cursor.execute('ALTER TABLE app_fundamentals ALTER COLUMN collected_date SET NOT NULL')
    cursor.execute('ALTER TABLE app_fundamentals DROP CONSTRAINT IF EXISTS app_fundamentals_pkey')
    cursor.execute('ALTER TABLE app_fundamentals ADD PRIMARY KEY (id)')

# Inicializar dados
def load_data():
    """Carrega ações e preferências do PostgreSQL."""
    data = {
        "stocks": [],
        "categories": {},
        "category_list": [],
    ...
    try:
        with psycopg2.connect(DB_URL, connect_timeout=10) as connection:
            with connection.cursor() as cursor:
                ensure_database_schema(cursor)

                cursor.execute('SELECT "Ação", "Categoria" FROM "Ação" ORDER BY "Ação"')
                stock_rows = cursor.fetchall()
                data["stocks"] = [stock for stock, _ in stock_rows]
                data["categories"] = {
                    stock: category for stock, category in stock_rows if category
                }

                cursor.execute('SELECT category FROM app_categories ORDER BY category')
                saved_categories = [row[0] for row in cursor.fetchall()]
                stock_categories = [category for _, category in stock_rows if category]
                data["category_list"] = list(dict.fromkeys(
                    saved_categories + stock_categories
                ))

                cursor.execute('SELECT name, field, operator, value FROM app_filters ORDER BY name')
                data["filters"] = {
                    name: {"field": field, "operator": operator, "value": value}
                    for name, field, operator, value in cursor.fetchall()
                }

                cursor.execute('SELECT name FROM app_columns ORDER BY position')
                saved_columns = [row[0] for row in cursor.fetchall()]
                if saved_columns:
                    data["selected_columns"] = saved_columns
    except psycopg2.Error as error:
        st.error(f"Erro ao carregar ações do PostgreSQL: {error}")

    return data

def save_data(data):
    """Salva ações e preferências no PostgreSQL."""
    try:
        with psycopg2.connect(DB_URL, connect_timeout=10) as connection:
            with connection.cursor() as cursor:
                ensure_database_schema(cursor)
                ...
                stock_rows = [
                    (stock, data.get("categories", {}).get(stock))
                    for stock in data.get("stocks", [])
                ]
                cursor.execute('DELETE FROM "Ação"')
                if stock_rows:
                    cursor.executemany(
                        'INSERT INTO "Ação" ("Ação", "Categoria") VALUES (%s, %s)',
                        stock_rows
                    )

                cursor.execute('DELETE FROM app_categories')
                categories = [(category,) for category in data.get("category_list", [])]
                if categories:
                    cursor.executemany(
                        'INSERT INTO app_categories (category) VALUES (%s)',
                        categories
                    )

                cursor.execute('DELETE FROM app_filters')
                filters = [
                    (name, config["field"], config["operator"], config["value"])
                    for name, config in data.get("filters", {}).items()
                ]
                if filters:
                    cursor.executemany(
                        'INSERT INTO app_filters (name, field, operator, value) VALUES (%s, %s, %s, %s)',
                        filters
                    )

                cursor.execute('DELETE FROM app_columns')
                columns = list(enumerate(data.get("selected_columns", [])))
                if columns:
                    cursor.executemany(
                        'INSERT INTO app_columns (position, name) VALUES (%s, %s)',
                        columns
                    )
        return True
...
def load_cache():
    """Carrega o cache de dados de ações do PostgreSQL."""
    try:
        with psycopg2.connect(DB_URL, connect_timeout=10) as connection:
            with connection.cursor() as cursor:
                ensure_database_schema(cursor)
                cursor.execute(
                    'SELECT cache_key, payload FROM app_cache WHERE updated_at >= %s',
                    (time.time() - CACHE_DURATION,)
                )
                return {
                    cache_key: pickle.loads(bytes(payload))
                    for cache_key, payload in cursor.fetchall()
                }
    except (psycopg2.Error, pickle.PickleError, EOFError) as error:
        st.warning(f"Não foi possível carregar o cache do PostgreSQL: {error}")
        return {}

def save_cache(cache):
    """Salva o cache de dados de ações no PostgreSQL."""
    with psycopg2.connect(DB_URL, connect_timeout=10) as connection:
        with connection.cursor() as cursor:
            ensure_database_schema(cursor)
            cursor.execute('DELETE FROM app_cache WHERE updated_at < %s', (time.time() - CACHE_DURATION,))
            for cache_key, cache_entry in cache.items():
                cursor.execute(
                    ...
                    (
                        cache_key,
                        psycopg2.Binary(pickle.dumps(cache_entry)),
...
def fetch_yahoo_chart_data(ticker_symbol, period_value, interval="1d"):
    """Busca histórico e metadados no endpoint Chart, que não exige crumb."""
    range_map = {
        "7d": "5d",
        "3y": "5y"
    }
    chart_range = range_map.get(period_value, period_value)
    ...
    try:
        ...
        if not timestamps:
            return pd.DataFrame(), {}, "O Yahoo Finance não retornou cotações"
        ...
        if adjclose:
            data["Adj Close"] = adjclose[0].get("adjclose", [])
        ...
        dataframe["Dividends"] = 0.0
        dataframe["Stock Splits"] = 0.0
        return dataframe.dropna(subset=["Close"]), result.get("meta", {}), None
...
def fetch_stock_data_with_retry(ticker, period_value, max_retries=3):
    """Busca dados pelo Chart API e usa yfinance como fallback."""
    dataframe, _, chart_error = fetch_yahoo_chart_data(ticker.ticker, period_value)
    ...
    dataframe, download_error = try_alternative_download(ticker.ticker, period_value)
    if not dataframe.empty:
        return dataframe, None

    return pd.DataFrame(), chart_error or download_error or "Não foi possível obter os dados"
...
def fetch_fundamental_indicators(ticker_symbol):
    """Busca e persiste indicadores fundamentalistas do Fundamentus."""
    fundamentus_ticker = ticker_symbol.upper().strip().removesuffix(".SA")
    if not fundamentus_ticker:
        return {}, "Informe um ticker válido."
    ...
    try:
        with psycopg2.connect(DB_URL, connect_timeout=10) as connection:
            with connection.cursor() as cursor:
                ...
                cursor.execute(
                    '''
                          SELECT pl, p_vp, dividend_yield, roe, divida_liquida_patrimonio,
                              margem_bruta, margem_ebit, crescimento_receita_5a
                    FROM app_fundamentals
                    WHERE ticker = %s AND collected_date = CURRENT_DATE
                    ''',
                    (fundamentus_ticker,)
                )
                saved_indicators = cursor.fetchone()
                if saved_indicators:
                    return {
                        "P/L": saved_indicators[0],
                        "P/VP": saved_indicators[1],
                        "Div. Yield": saved_indicators[2],
                        "ROE": saved_indicators[3],
                        "Dív. Líq / Patrim": saved_indicators[4],
                        "Marg. Bruta": saved_indicators[5],
                        "Marg. EBIT": saved_indicators[6],
                        "Cres. Rec (5a)": saved_indicators[7]
                    }, None
    except psycopg2.Error as error:
        return {}, f"Não foi possível verificar os indicadores salvos: {error}"
    ...
    for attempt in range(2):
        try:
            response = requests.get(
                ...
                params={"papel": fundamentus_ticker},
                headers={
            ...
            response.raise_for_status()
            soup = BeautifulSoup(
                response.content.decode(response.encoding or "latin-1"),
                "html.parser"
            )
            label_map = {
                "p/l": "P/L",
                ...
                "marg. bruta": "Marg. Bruta",
                "marg. ebit": "Marg. EBIT",
                "cres. rec (5a)": "Cres. Rec (5a)"
            }
            for label_node in soup.select("td.label .txt"):
                label = " ".join(label_node.get_text(" ", strip=True).split()).lower()
                value_cell = label_node.find_parent("td").find_next_sibling("td")
                if label in label_map and value_cell:
                    indicators[label_map[label]] = value_cell.get_text(" ", strip=True)
            if indicators:
                break
    ...
    if not indicators:
        ...
        return {}, f"Não foi possível localizar os indicadores do Fundamentus{detail}"

    def parse_fundamental_value(value):
        if value is None:
            return None
    ...
    indicators = {
        name: parse_fundamental_value(value)
    ...
    try:
        with psycopg2.connect(DB_URL, connect_timeout=10) as connection:
            with connection.cursor() as cursor:
                ...
                cursor.execute(
                    '''
                    INSERT INTO app_fundamentals
                        (ticker, collected_at, collected_date, pl, p_vp, dividend_yield, roe,
                         divida_liquida_patrimonio, margem_bruta, margem_ebit, crescimento_receita_5a)
                    VALUES (%s, CURRENT_TIMESTAMP, CURRENT_DATE, %s, %s, %s, %s, %s, %s, %s, %s)
                    ''',
                    (
                        fundamentus_ticker,
                        indicators.get("P/L"),
                        indicators.get("P/VP"),
                        indicators.get("Div. Yield"),
                        indicators.get("ROE"),
                        indicators.get("Dív. Líq / Patrim"),
                        indicators.get("Marg. Bruta"),
                        indicators.get("Marg. EBIT"),
                        indicators.get("Cres. Rec (5a)")
                    )
                )
    except psycopg2.Error as error:
        return indicators, f"Indicadores consultados, mas não foi possível armazená-los: {error}"

    return indicators, None

def load_saved_fundamental_indicators(ticker_symbol):
    """Carrega somente os indicadores já armazenados no banco."""
    fundamentus_ticker = ticker_symbol.upper().strip().removesuffix(".SA")
    try:
        with psycopg2.connect(DB_URL, connect_timeout=10) as connection:
            with connection.cursor() as cursor:
                ...
                cursor.execute(
                    '''
                          SELECT pl, p_vp, dividend_yield, roe, divida_liquida_patrimonio,
                              margem_bruta, margem_ebit, crescimento_receita_5a, collected_date
                    FROM app_fundamentals
                    WHERE REPLACE(UPPER(ticker), '.SA', '') = %s
                    ORDER BY collected_date DESC, collected_at DESC
                    LIMIT 1
                    ''',
                    (fundamentus_ticker,)
                )
        ...
        return {}, None, f"Não foi possível carregar os indicadores do banco: {error}"

    if not saved_indicators:
        return {}, None, "Nenhum indicador armazenado para este papel. Use o botão 'Atualizar Fundamentus'."
    ...
    return {
        ...
        "Marg. EBIT": saved_indicators[6],
        "Cres. Rec (5a)": saved_indicators[7]
    }, saved_indicators[8], None

def synchronize_daily_fundamentals(stocks):
    """Garante uma coleta diária dos indicadores de todas as ações cadastradas."""
    for stock in stocks:
        fetch_fundamental_indicators(stock)

def load_fundamental_history(ticker_symbol):
    """Carrega o histórico dos indicadores fundamentalistas de um ticker."""
    fundamentus_ticker = ticker_symbol.upper().strip().removesuffix(".SA")
    try:
        with psycopg2.connect(DB_URL, connect_timeout=10) as connection:
            with connection.cursor() as cursor:
                cursor.execute(
                    '''
                          SELECT collected_date, pl, p_vp, dividend_yield, roe, divida_liquida_patrimonio,
                              margem_bruta, margem_ebit, crescimento_receita_5a
                    FROM app_fundamentals
                    WHERE REPLACE(UPPER(ticker), '.SA', '') = %s
    ...
    history = pd.DataFrame(
        rows,
        columns=[
            "Data", "P/L", "P/VP", "Div. Yield", "ROE", "Dív. Líq / Patrim",
            "Marg. Bruta", "Marg. EBIT", "Cres. Rec (5a)"
        ]
    )
    if history.empty:
        return history, None
    ...
    for column in history.columns[1:]:
        history[column] = pd.to_numeric(history[column], errors="coerce").round(1)
...
def get_current_price(ticker_symbol):
    ...
    dataframe, metadata, _ = fetch_yahoo_chart_data(ticker_symbol, "1d", "1m")
    if not dataframe.empty:
        last_row = dataframe.iloc[-1]
        previous_close = metadata.get("chartPreviousClose") or metadata.get("previousClose")
        previous_close = previous_close or dataframe["Open"].dropna().iloc[0]
        return {
            'price': metadata.get("regularMarketPrice") or last_row['Close'],
            'previous_close': previous_close,
            'open': metadata.get("regularMarketOpen") or dataframe['Open'].dropna().iloc[0],
            'day_high': metadata.get("regularMarketDayHigh") or dataframe['High'].max(),
            'day_low': metadata.get("regularMarketDayLow") or dataframe['Low'].min(),
            'volume': metadata.get("regularMarketVolume") or dataframe['Volume'].sum(),
            'timestamp': dataframe.index[-1].to_pydatetime(),
            'currency': metadata.get("currency", 'BRL' if '.SA' in ticker_symbol.upper() else 'USD')
        }
...
st.caption("🗄️ Ações, preferências e cache armazenados no PostgreSQL")
...
with st.sidebar:
    ...
    if st.button(
        "Atualizar Fundamentus",
        type="primary",
        key="sync_fundamentals"
    ):
        if st.session_state.data["stocks"]:
            with st.spinner("Consultando indicadores fundamentalistas..."):
                synchronize_daily_fundamentals(st.session_state.data["stocks"])
            st.success("✅ Indicadores atualizados!")
        else:
            st.info("Cadastre pelo menos uma ação antes de atualizar os indicadores.")
...
def calcular_variacao(ticker_symbol, dias):
    """Calcula a variação percentual usando o endpoint Chart do Yahoo."""
    try:
        ...
        hist, _, _ = fetch_yahoo_chart_data(ticker_symbol, period)

        # Fallback para yfinance caso o endpoint Chart não responda.
        if hist.empty:
...
def calcular_variacao_ytd(ticker_symbol):
    """Calcula a variação desde o início do ano corrente (YTD)."""
    try:
        hist, _, _ = fetch_yahoo_chart_data(ticker_symbol, 'ytd')
...
if not st.session_state.data["stocks"]:
    ...
    with tab1:
        ...
        if selected_stock:
            ...
            fundamentus_data, fundamentals_date, fundamentus_error = load_saved_fundamental_indicators(selected_stock)
            indicator_names = [
                "P/L", "P/VP", "Div. Yield", "ROE", "Dív. Líq / Patrim",
                "Marg. Bruta", "Marg. EBIT", "Cres. Rec (5a)"
            ]
            if fundamentus_error:
                st.warning(f"⚠️ {fundamentus_error}")
            if fundamentus_data:
                def format_fundamental_value(indicator_name):
                    value = fundamentus_data.get(indicator_name)
                    if value is None or pd.isna(value):
                        return "N/A"
                    suffix = "%" if indicator_name in FUNDAMENTAL_PERCENTAGE_INDICATORS else ""
                    return f"{float(value):.1f}{suffix}"

                for row_start in range(0, len(indicator_names), 4):
                    indicator_columns = st.columns(4)
                    for column, indicator_name in zip(
                        indicator_columns, indicator_names[row_start:row_start + 4]
                    ):
                        with column:
                            st.metric(indicator_name, format_fundamental_value(indicator_name))
                st.caption(
                    "Fonte: Fundamentus | Papel consultado: "
                    f"{selected_stock.upper().strip().removesuffix('.SA')} | "
                    f"Dados do banco em {fundamentals_date}"
                )

            fundamental_history, history_error = load_fundamental_history(selected_stock)
            if history_error:
                st.warning(f"⚠️ Não foi possível carregar o histórico: {history_error}")
            elif not fundamental_history.empty:
                st.subheader("📈 Evolução dos Indicadores")
                history_indicator = st.selectbox(
                    "Indicador",
                    [
                        "P/L", "P/VP", "Div. Yield", "ROE", "Dív. Líq / Patrim",
                        "Marg. Bruta", "Marg. EBIT", "Cres. Rec (5a)"
                    ],
                    key="fundamental_history_indicator"
                )
                chart_data = fundamental_history[["Data", history_indicator]].dropna()
                if chart_data.empty:
                    ...
                    figure = go.Figure(
                        data=[go.Scatter(
                            x=chart_data["Data"],
                            y=chart_data[history_indicator],
                            mode="lines+markers",
                    ...
                    figure.update_layout(
                        ...
                        yaxis_title=history_indicator,
                        hovermode="x unified"
                    )
                    st.plotly_chart(figure, use_container_width=True)
                    st.dataframe(
                        fundamental_history.sort_values("Data", ascending=False).style.format({
                            indicator: "{:.1f}%" if indicator in FUNDAMENTAL_PERCENTAGE_INDICATORS else "{:.1f}"
    ...
    with tab3:
        ...
        with col_acoes:
            ...
            available_categories = st.session_state.data.get("category_list", [])
            new_stock_category = st.selectbox(
                "Categoria da Ação",
                available_categories,
                index=None,
                placeholder="Selecione uma categoria",
                key="manage_new_stock_category"
            )
            
            if st.button("Adicionar Ação", type="primary", key="manage_add_stock"):
                if not available_categories:
                    st.warning("⚠️ Cadastre uma categoria antes de adicionar uma ação!")
                elif not new_stock_category:
                    st.warning("⚠️ Selecione uma categoria para a ação!")
                elif new_stock and new_stock not in st.session_state.data["stocks"]:
                    st.session_state.data["stocks"].append(new_stock)
                    st.session_state.data.setdefault("categories", {})[new_stock] = new_stock_category
            ...
            if st.session_state.data["stocks"]:
                ...
                if stock_to_edit:
                    ...
                    # Seleção de categoria
                    current_cat = st.session_state.data.get("categories", {}).get(stock_to_edit)
                    categories_options = st.session_state.data.get("category_list", [])
                    cat_index = categories_options.index(current_cat) if current_cat in categories_options else None
                ...
                if category_to_edit:
                    ...
                    with col2:
                        if st.button("🗑️ Excluir Categoria", type="secondary", key="delete_edit_category"):
                            linked_stocks = [
                                stock for stock, cat in st.session_state.data.get("categories", {}).items()
                                if cat == category_to_edit
                            ]
                            if linked_stocks:
                                st.error(
                                    f"❌ Não é possível excluir '{category_to_edit}': "
                                    f"{len(linked_stocks)} ação(ões) ainda está(ão) vinculada(s)."
                                )
                            else:
